      ...
      <div onClick={this.props.onClick} className={`avalon-col-card-player-${playersCount} profile_details`}>
        <button className={`well profile_view avalon-card${selectable ? '' : '-unselectable'} ${side != null ? `avalon-${side ? 'good' : 'evil'}` : ''}`}>
          <div className="col-sm-12">
            ...
            {isMember ? <img src='/images/items/member.png' className="img-responsive avalon-mark-right"/> : ''}
            {isGuessed ? <img src='/images/items/selected.png' className="img-responsive avalon-mark-right"/> : ''}
          ...
          {status != '' ? <img src={`/images/items/${status.replace(' ', '-').toLowerCase()}.png`} className="img-responsive avalon-mark-bottom"/> : ''}
        </button>
      </div>
    );
  }
